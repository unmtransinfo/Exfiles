...
# Genes:
```{r message=FALSE}
genes <- read_delim("data/clue_genes_all.tsv", "\t", col_types = cols(entrez_id = col_character()))
setDT(genes)
...
```{r message=FALSE}
perts <- read_delim("data/clue_perts_all.tsv", "\t", col_types = cols(pubchem_cid = col_character()))
setDT(perts)
...
```{r message=FALSE}
pcls <- read_delim("data/clue_pcls.tsv", "\t")
setDT(pcls)
...
```{r message=FALSE}
cells <- read_delim("data/clue_cells_all.tsv", "\t")
setDT(cells)
